...
export class CirclesService {
  ...
  getUserProfileDetails(): Observable<AppUserProfileDetailsModel> {
    return this.http.get<AppUserProfileDetailsModel>(USERS_BASE_URL, {
      headers: {
  ...
  fetchGroupDetails(groupId: number): Observable<GroupDetailsItemModel> {
    return this.http.get<GroupDetailsItemModel>(`${GROUPS_BASE_URL}/${groupId}`);
  }

  joinGroup(groupId: number) : Observable<any>{
    return this.http.get(`${GROUPS_BASE_URL}/${groupId}/join`);
  }
}
